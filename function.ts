...
const factorial = (count: number): number | string => {
  if (count < 0) return 'Значение не может быть отрицательным';
...
//7. Напишите функцию, которая принимает массив в качестве входных данных и удаляет из него все "ложные значения"
const removeFalsyValues = <T>(array: T[]): T[] => {
  // использую дженерик прочитал про них. Чтобы функция сама определила свой тип
  // по приходящим в нее пропсам, и так мы не нарушим типизацию. Чтобы случайно не написать any
  return array.filter(Boolean) as T[]; // объект Boolean, который отфильтровывает ложные значения.
};
...
const equalityStroke = (strArray: string[]): boolean | null | string => {
  ...
  const result: boolean = strArray[0]
    .toLowerCase()
    .toUpperCase()
    .includes(strArray[1].toLowerCase().toUpperCase());
  // проверяем одинаковы ли строки приведенные к нижнему регистру и верхнему
...
const recourse = (array: number[], count: number) => {
  ...
  array.push(count);
  recourse(array, count - 1); // вызываем рекурсию
};
