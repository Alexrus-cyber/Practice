...
const removeFalsyValues = <T>(array: T[]): T[] => { // использую дженерик прочитал про них. Чтобы функция сама определлила свой тип
  return array.filter(Boolean) as T[];  // по приходящим в нее пропсам, и так мы не нарушим типизацию. Чтобы случайно не написать any
}; // объект Boolean, который отфильтровывает ложные значения.
